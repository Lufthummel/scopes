 ...
 * @param {string} [options.lumaType="709"] - either "709", "2020", "601" or "linear"
...
function Scope(source, target, options) {
  ...
  /**
   * The luma channel can be converted from RGB in various. It defaults
   * to industry standard BT.709. Also BT.2020, BT.601 and linear conversion
   * are supported. An unsupported name will default to "709".
   *
   * If no argument is given, current luma type is returned.
   *
   * NOTE: Although BT.2020 conversion is supported it is just an approximation
   * due to the browser being limit to 8-bit values.
   *
   * @param {string} [type="709"] - luma conversion scheme, valid values: 2020, 709 (default), 601, linear
