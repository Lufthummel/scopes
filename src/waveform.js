...
Scope._waveform = function(data, options) {
  ...
  function renderArray(ctx, r, g, b) {
    var
      idata = ctx.createImageData(w, h),          // todo create once and reuse
    ...
    for(y = 0; y < 256; y++) {
      ...
      pyi = ((h - 1) - y) * w;
      for(x = 0; x < w; x++) {
        var                                       // todo move decl. out of loop
          p = py + x,
          rr = Math.min(255, r[p] * intensity),   // todo cond. check might be faster than min()
